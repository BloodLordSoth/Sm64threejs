...
export class DustParticle {
    constructor(scene, maxParticles = 200) {
        ...
        this.map = this.maploader.load('./assets/processing/dust.png')
    ...
    emit(pos, count = 20) {
        for (let i = 0; i < count; i++) {
            ...
            this.positions[i3 + 0] = pos.x + (Math.random() - 0.5) * 0.5
            this.positions[i3 + 1] = pos.y - 0.5
            this.positions[i3 + 2] = pos.z + (Math.random() - 0.5) * 0.5

            this.velocities[i3 + 0] = (Math.random() - 0.5) * 0.05
            this.velocities[i3 + 1] = Math.random() * 0.03
